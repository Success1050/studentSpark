...
import { legalItems, navItems } from "@/constants";
            ...
            >
              ...
              <nav className="flex-1 px-4 mt-4 space-y-2">
                ...
                <div className="pt-4 pb-2 border-t border-zinc-100 dark:border-zinc-800">
                  <p className="px-4 text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-2">Legal & Help</p>
                  {legalItems.map((item) => (
                    <Link
                      key={item.label}
                      href={item.href}
                      onClick={() => setIsMobileMenuOpen(false)}
                      className={`flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${pathname === item.href
                          ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-500 shadow-sm border border-emerald-100 dark:border-emerald-500/20"
                          : "text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white"
                        }`}
                    >
                      <item.icon className="w-5 h-5 flex-shrink-0" />
        ...
        <nav className="flex-1 px-3 mt-4 space-y-1.5 overflow-y-auto custom-scrollbar">
          {navItems.map((item) => (
            <Link
              key={item.label}
              href={item.href}
              className={`flex items-center gap-3 px-3.5 py-3 rounded-2xl transition-all relative group ${pathname === item.href
                  ? "bg-indigo-50 dark:bg-indigo-500/10 text-indigo-500 shadow-sm border border-indigo-100 dark:border-indigo-500/20"
                  : "text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white"
                }`}
            >
          ...
          <div className="pt-6 pb-2 border-t border-zinc-100 dark:border-zinc-800">
            {isSidebarOpen && <p className="px-4 text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-4">Legal & Support</p>}
            {legalItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-2xl transition-all relative group ${pathname === item.href
                    ? "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-500 shadow-sm border border-emerald-100 dark:border-emerald-500/20"
                    : "text-zinc-500 dark:text-zinc-400 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:text-zinc-900 dark:hover:text-white"
                  }`}
              >
